...
public class ContaController {
    ...
    @FXML
    private TextField perfilNome;
    @FXML
    private TextField perfilEmail;
    @FXML
    private TextField perfilNif;
    @FXML
    private Label perfilErro;
    ...
    public void handleGuardarPerfil() {
        ...
        try {
            ...
            perfilErro.setText(e.getMessage());
        }
    }
    ...
    public void handleDepositar() {
        ...
        dlg.setTitle("Depositar");
        dlg.setHeaderText("Valor a depositar (€):");
        dlg.setContentText("Valor:");
    ...
    public void handleLevantar() {
        TextInputDialog dlg = new TextInputDialog("50");
        dlg.setTitle("Levantar");
        dlg.setHeaderText("Valor a levantar (€):");
        dlg.setContentText("Valor:");
        dlg.showAndWait().ifPresent(val -> {
            try {
    ...
    private VBox criarRowVeiculo(Veiculo v) {
        ...
        Label info = new Label("🚗 " + v.getNomeCompleto() + "  |  " + v.getLocalizacao() + "  |  "
                + String.format("%.2f€/dia", v.getPrecoPorDia()) + "  |  Estado: " + v.getEstado());
        ...
        btnRemover.setOnAction(e -> {
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION, "Remover " + v.getNomeCompleto() + "?",
                    ButtonType.YES, ButtonType.NO);
            confirm.showAndWait().ifPresent(bt -> {
                if (bt == ButtonType.YES) {
                    try {
                        veiculoService.removerVeiculo(v.getId());
                        carregarVeiculos();
                    } catch (Exception ex) {
    ...
    private void abrirModalVeiculo(Veiculo vExistente) {
        ...
        ComboBox<String> combustivel = new ComboBox<>();
        combustivel.getItems().addAll("Gasolina", "Gasóleo", "Elétrico", "GPL", "Híbrido");
        combustivel.setValue(vExistente != null ? vExistente.getCombustivel() : "Gasolina");
        ComboBox<String> transmissao = new ComboBox<>();
        transmissao.getItems().addAll("Manual", "Automática");
        transmissao.setValue(vExistente != null ? vExistente.getTransmissao() : "Manual");
        ...
        TextArea descricao = new TextArea(vExistente != null ? vExistente.getDescricao() : "");
                ...
                try {
                    ...
                    if (vExistente == null) {
                        veiculoService.adicionarVeiculo(marca.getText(), modelo.getText(), anoVal,
                                combustivel.getValue(), transmissao.getValue(), localizacao.getText(), precoVal,
                                consumoVal, descricao.getText());
    ...
    private void abrirModalEditar(Veiculo v) {
        abrirModalVeiculo(v);
    }
    ...
    private void abrirModalIndisponibilidade(Veiculo v) {
        ...
        btnAdd.setOnAction(e -> {
            try {
                ...
                recarregarListaIndisponibilidades(v, lista);
            } catch (Exception ex) {
                mostrarErro(ex.getMessage());
            }
        });
        ...
        VBox content = new VBox(10, new Label("Início:"), dpInicio, new Label("Fim:"), dpFim, btnAdd, new Separator(),
                new Label("Períodos definidos:"), lista);
        content.setPadding(new Insets(16));
    ...
    private void recarregarListaIndisponibilidades(Veiculo v, VBox lista) {
        ...
        try {
            veiculoService.listarIndisponibilidades(v.getId()).forEach(ind -> {
                ...
                rem.setOnAction(ev -> {
                    try {
                        veiculoService.removerIndisponibilidade(ind.getId());
                        recarregarListaIndisponibilidades(v, lista);
                    } catch (Exception ex) {
                        mostrarErro(ex.getMessage());
                    }
                ...
                javafx.scene.layout.HBox h = new javafx.scene.layout.HBox(8,
                        new Label(ind.getInicio() + " → " + ind.getFim()), rem);
    ...
    @FXML
    public void irParaVeiculos() {
        App.navigateTo("Vehicles");
    }

    @FXML
    public void irParaReservas() {
        App.navigateTo("Reservas");
    }

    @FXML
    public void irParaAdmin() {
        if (Session.isAdmin())
