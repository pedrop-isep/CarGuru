...
public class IndisponibilidadeRepository {
    ...
    public void save(Indisponibilidade ind) throws SQLException {
        String sql = "INSERT INTO indisponibilidades (veiculo_id, data_inicio, data_fim) VALUES (?,?,?)";
    ...
    public List<Indisponibilidade> findByVeiculo(int veiculoId) throws SQLException {
        ...
        String sql = "SELECT * FROM indisponibilidades WHERE veiculo_id=? ORDER BY data_inicio";
        try (Connection c = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                ind.setInicio(rs.getDate("data_inicio").toLocalDate());
                ind.setFim(rs.getDate("data_fim").toLocalDate());
                list.add(ind);
            }
        }
        return list;
    ...
    public boolean estaIndisponivel(int veiculoId, LocalDate inicio, LocalDate fim) throws SQLException {
        String sql = "SELECT COUNT(*) FROM indisponibilidades WHERE veiculo_id=? AND data_inicio < ? AND data_fim > ?";
        try (Connection c = DatabaseConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setInt(1, veiculoId);
